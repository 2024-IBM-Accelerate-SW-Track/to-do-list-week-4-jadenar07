...
class AddTodo extends Component {
  // Create a local react state of this component with both content date property set to nothing.
  constructor() {
    super();
    this.state = {
      content: "",
  ...
  // The handleSubmit function collects the form's input and puts it into the react state.
  // event.preventDefault() is called to prevent default event behavior like refreshing the browser.
  ...
  handleSubmit = (event) => {
    ...
    if (this.state.content.trim()) {
      const jsonObject = {
        ...
        dueDate: this.state.duedate ? this.state.duedate.toString() : null
      };

      // Making the POST request to the addItem function in the Express application
      Axios({
        method: "POST",
        url: "http://localhost:5001/add/item",
        data: jsonObject,
        headers: {
